...
const getNonNullUserParams = (body, user) => {
  ...
  newDataUser.name.firstName = body.name.firstName
    ? body.name.firstName
    : user.name.firstName;
  newDataUser.name.lastName = body.name.lastName
    ? body.name.lastName
    : user.name.lastName;
...
module.exports = {
  ...
  createUsersWithArrayInput: (req, res, next) => {
    if (req) console.log("NOT IMPLEMENTED");
      ...
      jwt.verify(token, process.env.JWT_SECRET, (errors, payload) => {
        ...
        if (payload)
          User.findById(payload.data).then(user => {
            // Check for a user with user ID from JWT payload
            if (user) {
              const userId = { data: user._id };
